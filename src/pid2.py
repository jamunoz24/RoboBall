...
wall_dist = 0.2             # Distance desired from the wall
max_speed = 0.22             # Maximum speed of the robot on meters/seconds
p = 15                      # Proportional constant for controller  
d = 0                       # Derivative coprnstant for controller 
...
def clbk_laser(msg):
    ...
    for i in range(0,359):
      ...
      if(translated[i] == 0):
        translated[i] = 10;
    regions_ = {
        'bright':  min(min(translated[180:240]), 10),
        ...
        'bleft':   min(min(translated[121:180]), 10),
    }
